...
export async function listCounselors() {
  const { rows } = await query(
    `SELECT id, first_name, last_name, name, salutation, room, specializations,
     ...
     FROM ssw_counselors WHERE active = TRUE ORDER BY last_name, first_name`
  );
  return rows;
}
...
export async function getCounselorById(id) {
  const { rows } = await query('SELECT * FROM ssw_counselors WHERE id = $1', [id]);
  if (!rows.length) {
    const err = new Error('Berater/in nicht gefunden');
    err.statusCode = 404;
    throw err;
  }
  return rows[0];
...
export async function listCategories() {
  const { rows } = await query(
    'SELECT id, name, description, icon FROM ssw_categories WHERE active = TRUE ORDER BY sort_order, id'
  );
  return rows;
}
